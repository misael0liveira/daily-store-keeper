...
function copyServiceWorkerToClient() {
  return {
    ...
    closeBundle() {
      ...
      try {
        const files = readdirSync(dist).filter((f) => f === "sw.js" || /^workbox-.*\.js$/.test(f));
...
export default defineConfig({
  ...
  plugins: [
    ...
    VitePWA({
      ...
      manifest: {
        id: "/",
        name: "Mercadinho União",
        short_name: "Mercadinho União",
        description: "Frente de caixa e gestão de estoque do Mercadinho União.",
        start_url: "/",
        scope: "/",
        display: "standalone",
